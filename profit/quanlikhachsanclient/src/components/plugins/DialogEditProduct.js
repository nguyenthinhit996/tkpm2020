...
import { editProduct } from '../../core/product'
...
export default function DialogEditProduct(props) {
    ...
    const { openEdit, setopenEdit } = useContext(NavigationAppContext);

    //save status new 
    var initalData = {...openEdit};
    ...
    const handleClose = () => {
        setopenEdit({ ...initalData, status: false });
        reset();
        history.go(0);
    };
        ...
        uploadTask.on("state_changed", console.log, console.error, () => {
            storage
                ...
                .then((url) => {
                    setFile(null);
                    // setURL(url);
                    openEdit.data.img = url;
                    setopenEdit({...openEdit});
                    console.log(openEdit.data.img);
                });
        });
    }
    // firebase end
    ...
    const onSubmit = async (data) => {
        console.log(data);
        if (openEdit.data.img.length !== 0) {
            let dataSendToServer = {
                idproduction: openEdit.data.idproduction,
                nameproduct: data.nameproduct,
                extention: data.extention,
                productrates: data.productrates,
                img: openEdit.data.img,
                type: refSelectNew.current.firstChild.value,
                status: "On"
            }
            console.log("update "+ JSON.stringify(data));

            let result = await editProduct(dataSendToServer);

            if (result) {
                exportToastSuccess("Update Product Success");
            } else {
    ...
    return (
        <div>
            <Dialog className="DialogEditProduct" aria-labelledby="customized-dialog-title" open={openEdit.status}>
                <DialogTitle className="font--Quanlikhachsan" id="customized-dialog-title" onClose={handleClose}>
                    {modalTitle}
                </DialogTitle>
                <DialogContent dividers>
                    <form className="DialogEditProductForm" onSubmit={handleSubmit(onSubmit)} >
                        <div className="form--mod--editProduct" >
                            <label > Name product:  </label>
                            <input name="nameproduct" defaultValue={openEdit.data.nameproduct}
                        ...
                        <div className="form--mod--editProduct" >
                            ...
                            {/* <input name="extention" defaultValue={openEdit.data.extention}
                                ref={register({ required: true })}
                            /> */}

                            <TextareaAutosize aria-label="minimum height" rowsMin={3} name="extention" defaultValue={openEdit.data.extention} 
                             ref={register({ required: true })} />
                        ...
                        <div className="form--mod--editProduct" >
                            ...
                            <input name="productrates" type="number" defaultValue={openEdit.data.productrates}
                                ref={register({ required: true })}
                            />
                        ...
                        <div className="form--mod--editProduct" >
                            ...
                            <NativeSelect
                                ...
                                defaultValue={openEdit.data.type}
                        ...
                        <Grid className="group-button-save-edit-product">
